...
import {
  ...
  Zap, Award, TrendingUp, Plus, Cpu, BusFront, Globe, Lock,
} from 'lucide-react'
...
function ProfilePhoto({ photo, onUpload }) {
  ...
  return (
    <div
      className="relative w-52 h-52 sm:w-64 sm:h-64 mx-auto lg:mx-0"
      ...
      <div className="relative w-full h-full rounded-2xl overflow-hidden bg-gradient-to-br from-[#0F1A2E] to-[#1a2744]">
        {photo ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center">
            ...
            <span className="text-xs text-[#4A6080] font-mono">Click to upload photo</span>
          </div>
        )}
        {/* Always-visible upload overlay on hover */}
        <div
          onClick={() => ref.current?.click()}
          className={`absolute inset-0 bg-black/60 flex flex-col items-center justify-center cursor-pointer transition-opacity duration-200 ${hov ? 'opacity-100' : 'opacity-0'}`}
        >
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-2">
        {repo.isPrivate ? <Lock size={14} className="text-[#4A6080] mt-0.5" /> : <Globe size={14} className="text-[#4A6080] mt-0.5" />}
...
// ── Hardcoded featured projects (shown when GitHub not configured) ──
const FEATURED_PROJECTS = [
  {
    ...
    description: 'Detects and classifies hand gestures from live webcam feed using MediaPipe for hand landmark detection and a custom CNN trained with PyTorch.',
  ...
  },
  {
    ...
    description: 'Real-time city bus tracking with WebSocket-powered location updates, Leaflet Map integration, and a Spring Boot REST backend.',
    ...
    >
      ...
      <div className="p-6">
        <div className="flex items-start justify-between mb-4">
          <span
            className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-mono font-600"
            style={{ background: `${project.accent}18`, color: project.accent, border: `1px solid ${project.accent}30` }}
          >
            {project.tag}
          </span>
          <div className="p-2 rounded-lg" style={{ background: `${project.accent}10` }}>
            <Icon size={20} style={{ color: project.accent }} />
          </div>
        </div>
        <h3 className="font-700 text-lg text-[#E8F0FE] mb-1">{project.name}</h3>
        <p className="text-sm font-600 mb-3" style={{ color: project.accent }}>{project.headline}</p>
        <p className="text-[#8EA4C8] text-sm leading-relaxed mb-4">{project.description}</p>
        <div className="flex flex-wrap gap-1.5 mb-4">
          {project.tech.map(t => <span key={t} className="tech-badge">{t}</span>)}
        </div>
        <div className="pt-4 border-t" style={{ borderColor: 'rgba(99,120,162,0.12)' }}>
          <a
            href={project.github}
            target="_blank"
            rel="noopener noreferrer"
...
// ── Custom projects section (localStorage) ──────
function CustomProjectsSection({ projects, onAdd }) {
  if (projects.length === 0) return null
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {projects.map((r, i) => (
        <MiniRepoCard key={r.id} repo={r} index={i} />
      ))}
    </div>
  )
}
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (stats.success && stats.data) {
        setGhStats(stats.data)
        setGithubConfigured(true)
      } else {
        setGithubConfigured(false)
      }
      if (rp.success && rp.data?.length) {
        setRepos(rp.data.filter(r => !r.isFork).slice(0, 6))
      }
    }).catch(() => {
      setGithubConfigured(false)
    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(180deg,#060D1F 0%,#0B1325 100%)' }}>
        ...
        <div className="container mx-auto px-4 sm:px-6 py-20 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center max-w-6xl mx-auto">
            ...
            <div className="animate-fade-in">
              <div className="section-badge mb-5 inline-flex">// backend · ai/ml · open source</div>

              <h1 className="font-display font-800 leading-tight mb-3" style={{ fontSize: 'clamp(38px,6vw,72px)' }}>
                <span className="text-[#8EA4C8] text-lg block mb-1 font-400 font-mono tracking-widest uppercase">Hello World, I'm</span>
                <span className="gradient-text block">{info.name}</span>
              </h1>
              ...
              <p className="text-[#8EA4C8] text-base leading-relaxed mb-6 max-w-lg">{info.bio}</p>
              ...
              <div className="mb-8">
                ...
                <p className="text-xs text-[#8EA4C8] font-mono">{info.stack}</p>
              </div>
              ...
              <div className="flex flex-wrap gap-3 mb-8">
                ...
                <Link href="/contact" className="btn-secondary">Hire Me</Link>
              </div>
            ...
            <div className="flex flex-col items-center gap-6 animate-slide-up delay-300">
              ...
              {!loading && !ghStats && (
                <div className="flex flex-wrap gap-2 justify-center">
                  <StatBadge icon={Award} value="8.11" label="CGPA" accent="#10B981" />
                  <StatBadge icon={Award} value="Top 5%" label="Rank" accent="#F59E0B" />
                  <StatBadge icon={Code2} value="6+" label="Certs" accent="#3B82F6" />
                  <StatBadge icon={TrendingUp} value="6+" label="OSS Orgs" accent="#8B5CF6" />
                </div>
              )}
      ...
      <section className="py-20 relative">
        <div className="container mx-auto px-4 sm:px-6">
          <div className="flex items-end justify-between mb-10">
            <div>
              <span className="section-badge mb-3 block w-fit">
                {githubConfigured && repos.length > 0 ? '// github · live sync' : '// featured projects'}
              </span>
              <h2 className="font-display font-800 text-[#E8F0FE]" style={{ fontSize: 'clamp(28px,4vw,44px)' }}>
                {githubConfigured && repos.length > 0 ? (
          ...
          {/* Show GitHub repos if available */}
          ...
          {/* Show hardcoded featured projects if GitHub not configured */}
          {!loading && repos.length === 0 && (
            <>
              ...
              <div className="glass-card p-5 text-center max-w-lg mx-auto"
                ...
                <Github size={20} className="text-[#4A6080] mx-auto mb-2" />
                <p className="text-xs text-[#4A6080] font-mono">
                  Add <code className="text-[#60A5FA]">GITHUB_TOKEN</code> to <code className="text-[#60A5FA]">.env.local</code> to auto-sync all GitHub repos
                </p>
              </div>
            </>
          )}
